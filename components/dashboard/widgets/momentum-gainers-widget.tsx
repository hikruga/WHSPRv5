...
interface MomentumGainersWidgetProps {
...
export function MomentumGainersWidget({ onCoinClick, gridLayout }: MomentumGainersWidgetProps) {
    ...
    >
      ...
      >
        {tokensData.slice(0, visibleTokens).map((token) => (
          <div
            key={token.symbol}
            className="flex items-center justify-between p-2 rounded-lg bg-gradient-to-r from-indigo-500/10 to-transparent border border-indigo-500/20 cursor-pointer hover:bg-indigo-500/20 transition-all duration-200"
            onClick={() => onCoinClick(token.symbol)}
          >
            <div className="flex items-center gap-2 min-w-0 flex-1">
              <div className="w-8 h-8 rounded-lg overflow-hidden border border-indigo-400 shadow-lg shadow-indigo-500/30 flex-shrink-0">
                <img
                  src={token.image}
                  alt={token.symbol}
            ...
            {selectedTokenForTrading === token.symbol ? (
              <div className="flex flex-col items-end gap-2 flex-shrink-0">
                ...
                <div className="flex items-center gap-2 bg-black/40 rounded-lg p-2 border border-indigo-500/20">
                  {/* Axiom */}
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <button
                ...
                <Button
                  size="sm"
                  variant="outline"
                  className="h-6 px-2 text-xs bg-indigo-500/10 border-indigo-500/30 text-indigo-400 hover:bg-indigo-500/20 cursor-pointer"
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <div className="text-right">
                  <div className="text-xs text-indigo-300">Volume</div>
                  <div className="text-xs text-indigo-400 font-semibold">{token.volume}</div>
                </div>
                <div className="text-right">
                  <div className="text-xs text-indigo-300">MC</div>
                  <div className="text-xs text-indigo-400 font-semibold">{token.mcap}</div>
                </div>
                <div className="text-right">
                  <div className="text-xs text-indigo-300">{token.change}</div>
                  <div className="text-xs text-indigo-400 font-semibold">{token.price}</div>
                </div>
                <Button
                  size="sm"
                  variant="outline"
