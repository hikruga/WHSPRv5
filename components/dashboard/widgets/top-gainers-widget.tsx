...
const tokensData = [
  {
    ...
    mcap: "$3.3M",
    volume: "$892K",
    image: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image-9JRY3Mxw2fB4DebD3gvEJTHTOeJHRk.png",
  ...
  },
  {
    ...
    mcap: "$22.1M",
    volume: "$4.2M",
    image: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image-xTqCUgyMuwfVYeFdR7JKOp0V9OZhjc.png",
  ...
  },
  {
    ...
    mcap: "$1.4M",
    volume: "$678K",
    image: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image-ptQjpixxwPt6HhwGynUTdHaPn53Vah.png",
  ...
  },
  {
    ...
    mcap: "$7.8B",
    volume: "$2.1B",
    image: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image-BbodFqJOPhaRNCpn00UUIHaYMBD0K5.png",
...
export function TopGainersWidget({ onCoinClick, gridLayout }: TopGainersWidgetProps) {
  ...
  // Handle clicking outside to close SVG window
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (selectedTokenForTrading) {
        const target = event.target as HTMLElement
        
        // Don't close if clicking on a button or inside the widget
        if (target.closest('button') || target.closest('[role="button"]')) {
          return
        }
        
        // Check if the click is outside the widget container
    ...
    >
      ...
      >
        {tokensData.slice(0, visibleTokens).map((token) => (
          <div
            key={token.symbol}
            className="flex items-center justify-between p-2 rounded-lg bg-gradient-to-r from-cyan-500/10 to-transparent border border-cyan-500/20 cursor-pointer hover:bg-cyan-500/20 transition-all duration-200 min-w-0"
          ...
          >
              ...
              {selectedTokenForTrading === token.symbol ? (
                <div className="flex flex-col items-end gap-2 flex-shrink-0 min-w-0">
                  ...
                  <div className="flex items-center gap-1 bg-black/40 rounded-lg p-1 border border-cyan-500/20">
                    {/* Axiom */}
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button
                          onClick={(e) => handlePlatformClick('https://axiom.trade', token, e)}
                          className="p-1 bg-purple-500/20 hover:bg-purple-500/30 border border-purple-500/30 rounded transition-colors"
                        >
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button
                          ...
                          className="p-1 bg-orange-500/20 hover:bg-orange-500/30 border border-orange-500/30 rounded transition-colors"
                        >
                          <img src="/Bullx-logo.svg" alt="Bull X" width={14} height={14} />
                        </button>
                      </TooltipTrigger>
                      <TooltipContent side="top">
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button
                          ...
                          className="p-1 bg-blue-500/20 hover:bg-blue-500/30 border border-blue-500/30 rounded transition-colors"
                        >
                          <img src="/photon logo.svg" alt="Photon" width={14} height={14} />
                        </button>
                      </TooltipTrigger>
                      <TooltipContent side="top">
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button
                          ...
                          className="p-1 bg-green-500/20 hover:bg-green-500/30 border border-green-500/30 rounded transition-colors"
                        >
                          <img src="/gmgn-logo.svg" alt="GMGN" width={14} height={14} />
                        </button>
                      </TooltipTrigger>
                      <TooltipContent side="top">
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button
                          ...
                          className="p-1 bg-indigo-500/20 hover:bg-indigo-500/30 border border-indigo-500/30 rounded transition-colors"
                        >
                          <img src="/nova-logo.svg" alt="Nova" width={14} height={14} />
                        </button>
                      </TooltipTrigger>
                      <TooltipContent side="top">
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button
                          ...
                          className="p-1 bg-yellow-500/20 hover:bg-yellow-500/30 border border-yellow-500/30 rounded transition-colors"
                        >
                          <svg width="14" height="14" viewBox="0 0 252 300" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className="flex items-center gap-1 flex-shrink-0 min-w-0">
                  <div className="text-right min-w-0">
                    <div className="text-xs text-cyan-300 truncate">Vol</div>
                    <div className="text-xs text-cyan-400 font-semibold truncate">{token.volume}</div>
                  </div>
                  <div className="text-right min-w-0">
                    <div className="text-xs text-cyan-300 truncate">MC</div>
                    <div className="text-xs text-cyan-400 font-semibold truncate">{token.mcap}</div>
                  </div>
                  <div className="text-right min-w-0">
                    <div className="text-xs text-cyan-300 truncate">{token.change}</div>
                    <div className="text-xs text-cyan-400 font-semibold truncate">{token.price}</div>
                  </div>
                  <Button
                    size="sm"
                    variant="outline"
                    className="h-6 px-2 text-xs bg-cyan-500/10 border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/20 cursor-pointer flex-shrink-0"
                    onClick={(e) => handleTradeClick(token.symbol, e)}
                    onMouseDown={(e) => handleTradeClick(token.symbol, e)}
                  >
                    Trade
