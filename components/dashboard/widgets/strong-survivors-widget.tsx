...
interface StrongSurvivorsWidgetProps {
...
export function StrongSurvivorsWidget({ onCoinClick, gridLayout }: StrongSurvivorsWidgetProps) {
    ...
    >
      ...
      >
        {tokensData.slice(0, visibleTokens).map((token) => (
          <div
            key={token.symbol}
            className="flex items-center justify-between p-2 rounded-lg bg-gradient-to-r from-emerald-500/10 to-transparent border border-emerald-500/20 cursor-pointer hover:bg-emerald-500/20 transition-all duration-200"
            onClick={() => onCoinClick(token.symbol)}
          >
              <div className="flex items-center gap-2 min-w-0 flex-1">
                <div className="w-8 h-8 rounded-lg overflow-hidden border border-emerald-400 shadow-lg shadow-emerald-500/30 flex-shrink-0">
                  <img
                    src={token.image}
                    alt={token.symbol}
                      ...
                      <TooltipTrigger asChild>
                        ...
                        >
                          <svg width="16" height="16" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M24.1384 17.3876H11.8623L18.0001 7.00012L24.1384 17.3876Z" fill="#10B981"/>
                            <path d="M31 29.0003L5 29.0003L9.96764 20.5933L26.0324 20.5933L31 29.0003Z" fill="#10B981"/>
                          </svg>
                        </button>
                      </TooltipTrigger>
                      <TooltipContent side="top">
