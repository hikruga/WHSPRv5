...
export function BigDipsWidget({ onCoinClick, gridLayout }: BigDipsWidgetProps) {
    ...
    >
      ...
      >
        {tokensData.slice(0, visibleTokens).map((token) => (
          <div
            key={token.symbol}
            className="flex items-center justify-between p-2 rounded-lg bg-gradient-to-r from-orange-500/10 to-transparent border border-orange-500/20 cursor-pointer hover:bg-orange-500/20 transition-all duration-200 min-w-0"
            onClick={() => onCoinClick(token.symbol)}
          >
            <div className="flex items-center gap-2 min-w-0 flex-1 overflow-hidden">
              <div className="w-8 h-8 rounded-lg overflow-hidden border border-orange-400 shadow-lg shadow-orange-500/30 flex-shrink-0">
                <img
                  src={token.image}
                  alt={token.symbol}
            ...
            {selectedTokenForTrading === token.symbol ? (
              <div className="flex flex-col items-end gap-2 flex-shrink-0 min-w-0">
                ...
                <div className="flex items-center gap-1 bg-black/40 rounded-lg p-1 border border-orange-500/20">
                  {/* Axiom */}
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <button
                      ...
                      >
                        <svg width="14" height="14" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M24.1384 17.3876H11.8623L18.0001 7.00012L24.1384 17.3876Z" fill="#F97316"/>
                          <path d="M31 29.0003L5 29.0003L9.96764 20.5933L26.0324 20.5933L31 29.0003Z" fill="#F97316"/>
                        </svg>
                      </button>
                    </TooltipTrigger>
                    <TooltipContent side="top">
                ...
                <Button
                  size="sm"
                  variant="outline"
                  className="h-6 px-2 text-xs bg-orange-500/10 border-orange-500/30 text-orange-400 hover:bg-orange-500/20 cursor-pointer"
              ...
              <div className="flex items-center gap-1 flex-shrink-0 min-w-0">
                <div className="text-right min-w-0">
                  <div className="text-xs text-orange-300 truncate">Vol</div>
                  <div className="text-xs text-orange-400 font-semibold truncate">{token.volume}</div>
                </div>
                <div className="text-right min-w-0">
                  <div className="text-xs text-orange-300 truncate">MC</div>
                  <div className="text-xs text-orange-400 font-semibold truncate">{token.mcap}</div>
                </div>
                <div className="text-right min-w-0">
                  <div className="text-xs text-orange-300 truncate">{token.change}</div>
                  <div className="text-xs text-orange-400 font-semibold truncate">{token.price}</div>
                </div>
                <Button
                  size="sm"
                  variant="outline"
                  className="h-6 px-2 text-xs bg-orange-500/10 border-orange-500/30 text-orange-400 hover:bg-orange-500/20 cursor-pointer flex-shrink-0"
                  onClick={(e) => handleTradeClick(token.symbol, e)}
                  onMouseDown={(e) => handleTradeClick(token.symbol, e)}
                >
                  Trade
